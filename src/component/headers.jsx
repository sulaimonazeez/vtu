...
const Header = () => {
  ...
  const fetchProfileAndBalance = useCallback(async () => {
    try {
      ...
      console.log("Profile & Balance Fetched");
      ...
      if (error.response?.status === 401) await handleRefresh();
    }
  }, [navigate]);
  ...
  useEffect(() => {
    const checkTokenAndFetch = async () => {
      ...
      if (Date.now() >= expiresIn) await handleRefresh();
      await fetchProfileAndBalance();
    };

    checkTokenAndFetch();
  }, [fetchProfileAndBalance, handleRefresh]);
